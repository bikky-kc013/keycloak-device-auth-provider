...
<@layout.registrationLayout displayInfo=false displayMessage=false; section>
    <#if section = "form">
        ...
        <div class="sa-body">
            ...
            <form id="kc-otp-form" action="${url.loginAction}" method="post" novalidate>
                <div class="sa-otp-group<#if message?has_content> has-error</#if>" id="otpBoxes" role="group"
                     aria-label="${msg("otpLabel")!"Enter the code we sent you"}">
                    <#list 1..6 as i>
                        <input type="text" inputmode="numeric" pattern="[0-9]*" maxlength="1" class="sa-otp-box"
                               autocomplete="one-time-code" aria-label="${msg("otpDigitLabel", i)!("Digit " + i + " of 6")}"
                               <#if i == 1>autofocus</#if> />
                    </#list>
                </div>
                ...
                <noscript>
                    <div class="${properties.kcFormGroupClass!}">
                        <label for="otpFallback" class="${properties.kcLabelClass!}">${msg("otpLabel")!"Enter the code we sent you"}</label>
                        <input type="text" id="otpFallback" name="otp" class="${properties.kcInputClass!}"
